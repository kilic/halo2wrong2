use super::{fq2::Fq2, fq6::Fq6, BLSPairingChip};
use crate::circuitry::stack::Stack;
use ff::PrimeField;
use halo2::{
    circuit::Value,
    halo2curves::bls12381::{self},
};
...
impl<N: PrimeField> Fq12<N> {
    pub fn value(&self) -> Value<bls12381::Fq12> {
        self.c0
            .value()
            .zip(self.c1.value())
            .map(|(c0, c1)| bls12381::Fq12 { c0, c1 })
    }
}
...
impl<N: PrimeField + Ord> BLSPairingChip<N> {
    ...
    // pub(crate) fn fq12_constant(&self, stack: &mut Stack<N>, e: bls12381::Fq12) -> Fq12<N> {
    //     Fq12 {
    ...
    pub(crate) fn fq12_conjugate(&self, stack: &mut Stack<N>, a: &Fq12<N>) -> Fq12<N> {
    ...
    // pub(crate) fn fq12_assign(&self, stack: &mut Stack<N>, e: Value<bls12381::Fq12>) -> Fq12<N> {
    //     Fq12 {
    ...
    pub(crate) fn fq12_mul_sparse(
        ...
        let t1 = self.fq6_mul_by_01(stack, &f.c1, c0, c1);
        let v0 = self.fq6_mul_by_non_residue(stack, &f.c1);
        let v0 = self.fq6_mul_by_non_residue(stack, &v0);
        let v1 = self.fq6_mul_by_non_residue(stack, &f.c0);
        f.c0 = self.fq6_add(stack, &t0, &v0);
        f.c1 = self.fq6_add(stack, &t1, &v1);
    }
