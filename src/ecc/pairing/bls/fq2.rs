...
use halo2::{circuit::Value, halo2curves::bls12381};

#[derive(Clone, Debug)]
pub struct Fq2<N: PrimeField> {
    c0: Integer<bls12381::Fq, N>,
    c1: Integer<bls12381::Fq, N>,
}

impl<N: PrimeField> Fq2<N> {
    pub fn value(&self) -> Value<bls12381::Fq2> {
        self.c0
            .value()
            .zip(self.c1.value())
            .map(|(c0, c1)| bls12381::Fq2::new(c0, c1))
    }
}
...
impl<N: PrimeField + Ord> BLSPairingChip<N> {
    pub fn fq2_assign(
        &self,
        stack: &mut Stack<N>,
        e: Value<halo2::halo2curves::bls12381::Fq2>,
    ) -> Fq2<N> {
    ...
    pub(crate) fn fq2_one(&self, stack: &mut Stack<N>) -> Fq2<N> {
        let one = self.ch.register_constant(stack, &bls12381::Fq::ONE);
        let zero = self.ch.register_constant(stack, &bls12381::Fq::ZERO);
        Fq2 {
            c0: one.clone(),
            c1: zero.clone(),
        }
    ...
    pub(crate) fn fq2_constant(&self, stack: &mut Stack<N>, constant: bls12381::Fq2) -> Fq2<N> {
    ...
    pub(crate) fn fq2_mul_by_non_residue(&self, stack: &mut Stack<N>, a: &Fq2<N>) -> Fq2<N> {
        let c0 = self.ch.sub(stack, &a.c0, &a.c1);
        let c1 = self.ch.add(stack, &a.c0, &a.c1);
        Fq2 { c0, c1 }
    }
