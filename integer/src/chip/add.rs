...
use circuitry::witness::{Scaled, Witness};
use ff::PrimeField;
    ...
    > IntegerChip<W, N, NUMBER_OF_LIMBS, LIMB_SIZE, NUMBER_OF_SUBLIMBS, SUBLIMB_SIZE>
{
    pub fn add(
        ...
        {
            let c = a.max() + b.max();
            assert!(self.rns.max_unreduced_value > c);
        }
    ...
    pub fn sub(
        ...
        {
            let c = a.max() + b.max();
            assert!(self.rns.max_unreduced_value > c, "pre aux");
        }

        let (aux_witness, aux_big, aux_nat) = self.subtracion_aux(&b.max_vals);

        #[cfg(feature = "synth-sanity")]
        {
            let c = a.max() + compose::<NUMBER_OF_LIMBS, LIMB_SIZE>(&aux_big);
            assert!(self.rns.max_unreduced_value > c, "post aux");
        }
    ...
    pub fn neg(
        ...
        let (aux_witness, aux_big, aux_nat) = self.subtracion_aux(&a.max_vals);
