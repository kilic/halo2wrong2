...
impl<'a, 'b, F: PrimeField> RegionCtx<'a, 'b, F> {
    ...
    pub fn fixed(&mut self, column: Column<Fixed>, value: F) -> Result<(), Error> {
        self.region.assign_fixed(column, self.offset(), value);
        Ok(())
    }
    ...
    pub fn advice(
        ...
        // self.region.assign_advice(column, self.offset(), value);
        let u = self.region.assign_advice(column, self.offset, value);

        let cell = u.cell();
        let value = u.value().cloned();

        let value = value.map(|e| match e {
            Assigned::Trivial(e) => *e,
            _ => panic!(),
        });
    ...
    fn empty(&mut self, column: Column<Any>) -> Result<(), Error> {
        match column.column_type() {
            Any::Advice(_) => {
                self.advice(column.try_into().unwrap(), Value::known(F::zero()))?;
                Ok(())
            }

            Any::Fixed => {
                self.fixed(column.try_into().unwrap(), F::zero())?;
                Ok(())
            }
            _ => panic!("Cannot assign to instance column"),
        }
...
//     //         Any::Advice(_) => {
//     //             self.region
//     //                 .assign_advice(column.try_into().unwrap(), self.offset, || {
//     //                     Value::known(F::zero())
//     //                 })
//     //         }
//     //         Any::Fixed => self
//     //             .region
//     //             .assign_fixed(column.try_into().unwrap(), self.offset, || {
//     //                 Value::known(F::zero())
//     //             }),
//     //         _ => panic!("Cannot assign to instance column"),
//     //     }
