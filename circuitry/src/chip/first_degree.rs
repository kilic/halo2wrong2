use crate::witness::{Composable, Scaled, Witness};
use ff::PrimeField;

use super::Chip;

pub trait FirstDegreeChip<F: PrimeField + Ord>:
    Chip<crate::enforcement::FirstDegree<F>, F>
{
    ...
    fn zero_sum(&mut self, terms: &[Scaled<F>], constant: F) {
        ...
        let composition = crate::enforcement::FirstDegree::new(terms, constant);
    ...
    fn compose(&mut self, terms: &[Scaled<F>], constant: F) -> Witness<F> {
        ...
        terms.push(result);
        let composition = crate::enforcement::FirstDegree::new_no_range(terms, constant);
        self.new_op(composition);
        result.witness
    }
}
